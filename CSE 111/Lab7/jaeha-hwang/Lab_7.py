...
def Q1(_conn):
    ...
    try:
        ...
        l = '{:>10}{:>6}{:>46}{:>11}{:>11}'.format("wId", "wName", "wCap", "sId", "nId")
        print(l, file = output)
        for x in result:
            l = '{:>10} {:<35} {:>15} {:>10} {:>10}'.format(x[0],x[1],x[2],x[3],x[4])
            print (l, file = output)
        output.close()
    except Error as e:
        _conn.rollback()
...
def Q2(_conn):
    ...
    print("Q2")
    try:
        sql = """SELECT n_name, COUNT(w_nationkey), SUM(w_capacity) FROM nation, warehouse
        WHERE n_nationkey = w_nationkey
        GROUP BY n_name
        ORDER BY COUNT(w_nationkey) DESC, SUM(w_capacity) DESC, n_name DESC"""
        ...
        l = '{:>6}{:>45}{:>10}'.format('nation','numW','totCap')
        print (l, file = output)
        for x in result:
            l = '{:<15}{:>36}{:>10}'.format(x[0],x[1],x[2])
            print (l, file = output)
        output.close()
    except Error as e:
        _conn.rollback()
...
def Q3(_conn):
    ...
    try:
        ...
        input1 = inputfile.read().splitlines()
        
        sql = """
        SELECT s_name, n_name, w_name FROM supplier, nation, warehouse
        WHERE s_nationkey = n_nationkey AND s_suppkey = w_suppkey
        AND (substr(w_name, length(w_name) - 4, length(w_name)) = (?))"""
        ...
        l = '{:<10}{:>17}{:>24}'.format('supplier','nation','warehouse')
        print(l, file = output)
        for x in result:
            l = '{:<21}{:<21}{:<30}'.format(x[0],x[1],x[2])
            print(l, file = output)
        output.close()
        inputfile.close()
    except Error as e:
...
def Q4(_conn):
    ...
    try:
        ...
        INNER JOIN nation ON nation.n_nationkey = warehouse.w_nationkey
        INNER JOIN region ON region.r_regionkey = nation.n_regionkey
        WHERE r_name = (?) AND w_capacity > (?)
        ORDER BY w_capacity DESC"""
        
        cur = _conn.cursor()
        cur.execute(sql, (inputlist[0], inputlist[1]))
        result = cur.fetchall()
        output = open('output/4.out', 'w')

        l = '{:<43} {:<10}'.format('warehouse', 'capacity')
        print (l, file = output)
        for x in result:
            l = '{:<47} {:<10}'.format(x[0],x[1])
            print(l, file = output)
        output.close()
        inputfile.close()
    except Error as e:
...
def Q5(_conn):
    ...
    try:
        ...
        INNER JOIN nation n1 ON n1.n_regionkey = region.r_regionkey
        LEFT JOIN (SELECT * FROM warehouse
            INNER JOIN supplier ON supplier.s_suppkey = warehouse.w_suppkey
            INNER JOIN nation n2 ON n2.n_nationkey = supplier.s_nationkey
            WHERE n_name = ?) AS asdf ON asdf.w_nationkey = n1.n_nationkey
        GROUP BY r_name"""
        ...
        l = '{:<33} {:<10}'.format('region', 'capacity')
        print (l, file = output)
        for x in result:
            l = '{:<30}  {:>10}'.format(x[0],x[1])
            print(l, file = output)
        output.close()
        inputfile.close()
    except Error as e:
