...
int main(int argc, char **argv){
    int myrank, nproc, buf[DIM_BUF], i, offset, intsize;
    MPI_File fh;
    MPI_Status status;
    ...
    MPI_Type_vector(DIM_BUF/2,2,2*nproc, MPI_INT, &datatype);
    MPI_Type_commit(&datatype);

    MPI_Type_size(MPI_INT, &intsize);
    offset = 2 * intsize * myrank;
    MPI_File_set_view(fh, offset, MPI_INT, datatype, "native", MPI_INFO_NULL);
