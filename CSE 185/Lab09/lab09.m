...
for index = 1:k
    pair = randperm(length(matches),1);
    
    p1 = f1(1:2, matches(1, pair));
    p2 = f2(1:2, matches(2, pair));
    tx0 = p1(1) - p2(1);
    ty0 = p1(2) - p2(2);
    inlier = 0;
    
    for other = 1:length(matches)
        if other ~= pair
            p1 = f1(1:2, matches(1, other));
            p2 = f2(1:2, matches(2, other));
            tx1 = p1(1) - p2(1);
            ty1 = p1(2) - p2(2);
            
            if (tx1-tx0)^2 + (ty1-ty0)^2 < delta
                inlier = inlier + 1;
            end
        end
    end
