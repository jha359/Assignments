...
{
    /* Your code goes here */
    //Note to self: int instr is hexadecimal
...
void PrintInstruction ( DecodedInstr* d) {
    /* Your code goes here */
    if(d->op == 3) //J-format instructions
    {
        printf("jal\t0x%x\n", d->regs.j.target);
    }
    else if(d->op == 2)
    ...
    else if(d->op == 0) //R-format instructions
    {
    ...
    {
        int address = mips.pc + 4 + (d->regs.i.addr_or_immed * 4);
        printf("beq\t$%d, $%d, 0x%x\n", d->regs.i.rs, d->regs.i.rt, address);
    }
    else if(d->op == 5)
    {
        int address = mips.pc + 4 + (d->regs.i.addr_or_immed * 4);
...
int Execute ( DecodedInstr* d, RegVals* rVals) {
    /* Your code goes here */
    if(d->op == 0)//R-Format Instructions
    {
        if(d->regs.r.funct == 33) //addu
        {
            return rVals->R_rd = rVals->R_rs + rVals->R_rt; 
        }
        ...
        {
            return rVals->R_rd = rVals->R_rt << d->regs.r.shamt;
        }
    ...
    else if(d->op == 43 || d->op == 35)//sw and lw
    {
...
void UpdatePC ( DecodedInstr* d, int val) {
    ...
    if(d->op == 0 && d->regs.r.funct == 8)//jr
    {
        mips.pc = val;
    }
    else if(d->op == 2 || d->op == 3)//j and jal
    {
        mips.pc = d->regs.j.target;
    }
    else if(d->op == 4 || d->op == 5)//beq and bne
    {
        if(val == 1)
        {
...
int Mem( DecodedInstr* d, int val, int *changedMem) {
    /* Your code goes here */
    if(d->op == 43 || d->op == 35)//error check
    {
        ...
        else
        {
            ...
            {
                *changedMem = val;
                return mips.memory[(val - 0x00400000) / 4] = mips.registers[d->regs.i.rt];
            }
            else if(d->op == 35) //lw
            {
                *changedMem = -1;
